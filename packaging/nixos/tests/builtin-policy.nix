...
let
  ...
  # Not common.baseNode: it grants everything through polkit.
  builtinNode =
    { ... }:
    {
...
pkgs.testers.nixosTest {
  name = "defused-builtin-policy-${kernelPackages.kernel.version}";
  ...
  nodes = {
    ...
    allowOther =
      ...
      {
        imports = [ builtinNode ];
        services.defused.allowPrivilegedFlags = [ "allow_other" ];
      };
  };

  testScript =
    { nodes, ... }:
    # The module must not pull polkit in for the built-in policy.
    assert !nodes.restricted.security.polkit.enable;
    assert !nodes.allowOther.security.polkit.enable;
    ''
      start_all()
      ...
      with subtest("the unit carries the built-in policy and nothing wants polkit"):
          restricted.succeed(
              "systemctl cat defused@.service | grep -F -- '--policy=builtin'"
          )
          restricted.succeed(
              "systemctl cat defused@.service | grep -F -- '--allow-groups=fusers'"
          )
          restricted.fail("systemctl cat defused@.service | grep -F polkit")
          allowOther.succeed(
              "systemctl cat defused@.service | grep -F -- '--allow-privileged-flags=allow_other'"
          )
          restricted.fail("systemctl is-active polkit.service")
      ...
      with subtest("allow_other is refused unless --allow-privileged-flags grants it"):
          restricted.succeed(
              "timeout 45s runuser -u alice -- "
              "${pkgs.python3}/bin/python3 ${mountHelper} "
