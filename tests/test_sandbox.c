...
/* Pointer arguments exist only once the probe has pinned them, so it names
 * one. The values sit far above any literal a probe passes. */
enum {
...
#define ARGS(...) ((const long[6]){__VA_ARGS__})
...
static const struct probe reply_probes[] = {
    ...
    {"writev to stderr, as the logger does", ALLOW, SYS_writev,
     ARGS(STDERR_FILENO, ARG_HANDLE_COPY, 0)},
...
static const struct probe mount_probes[] = {
    {"move_mount as the process makes it", ALLOW, SYS_move_mount,
    ...
    {"setns as the process makes it", ALLOW, SYS_setns,
    ...
    {"getpid, which the process never makes", DENY, SYS_getpid, ARGS(0)},
    ...
    {"openat, which the process never makes", DENY, SYS_openat,
     ARGS(FD_PARENT, ARG_PATH, O_RDONLY)},
};

static const struct probe unmount_probes[] = {
    {"fchdir as the process makes it", ALLOW, SYS_fchdir, ARGS(FD_PARENT)},
    {"fchdir on another descriptor", DENY, SYS_fchdir, ARGS(FD_PIDFD)},
    {"name_to_handle_at as the process makes it", ALLOW, SYS_name_to_handle_at,
    ...
    {"umount2 as the process makes it", ALLOW, SYS_umount2,
     ARGS(ARG_PATH, UMOUNT_NOFOLLOW)},
    ...
    {"openat, which the process never makes", DENY, SYS_openat,
     ARGS(FD_PARENT, ARG_PATH, O_RDONLY)},
    {"read, which the process never makes", DENY, SYS_read,
     ARGS(FD_PARENT, ARG_HANDLE_COPY, 1)},
    {"close, which the process never makes", DENY, SYS_close, ARGS(FD_PARENT)},
};
...
/* The filter leaves no way to report but stderr, so a probe reports
...
static int run_probe(const struct probe *p, enum defused_op op, bool filtered) {
    ...
    if (pid == 0) {
        ...
        errno = 0;
        long ret = syscall(p->nr, a[0], a[1], a[2], a[3], a[4], a[5]);
...
static void test_filter(void) {
    int skipped =
        ...
        run_probes(reply_probes, ARRAY_SIZE(reply_probes), DEFUSED_OP_MOUNT) +
        run_probes(unmount_probes, ARRAY_SIZE(unmount_probes),
                   DEFUSED_OP_UNMOUNT) +
        run_probes(reply_probes, ARRAY_SIZE(reply_probes), DEFUSED_OP_UNMOUNT);
    if (skipped)
        fprintf(stderr,
